...
num_folds = 4
group_assignment = np.random.randint(0, num_folds, size=len(unique_indices))
group_dict = dict(zip(unique_indices, group_assignment))
fold_numbers = [group_dict[idx] for idx in index]
ps = PredefinedSplit(test_fold=fold_numbers)

param_dist = {'alpha': uniform()}
lasso = Lasso()

# Initialize the grid search model
grid_search = RandomizedSearchCV(estimator=lasso,
