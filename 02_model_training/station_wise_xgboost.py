...
index = train_set.index.get_level_values(0)
# Set the random seed for reproducibility
np.random.seed(42)
# Get the unique indices from the dataset's index and convert to an array
unique_indices = np.array(index.unique())
# Shuffle the unique indices to ensure randomness
np.random.shuffle(unique_indices)

# Allocate folds
num_folds = 4
# Initially assign an equal number of unique indices to each fold
fold_sizes = np.full(num_folds, len(unique_indices) // num_folds, dtype=int)
# Distribute any remaining indices (if the total number isn't evenly divisible by the number of folds)
fold_sizes[:len(unique_indices) % num_folds] += 1
current = 0
group_dict = {}
# Assign each unique index to a fold
for fold_size, fold_number in zip(fold_sizes, range(num_folds)):
    for i in range(fold_size):
        group_dict[unique_indices[current]] = fold_number
        current += 1

# Assign a fold number to each row in the dataset based on its index
fold_numbers = [group_dict[idx] for idx in index]

# Create a PredefinedSplit object for cross-validation
